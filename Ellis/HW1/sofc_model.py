...
if params.dae_flag:
    ...
    algvars = np.concatenate((ptr.phi_an_el[1:], ptr.phi_elyte))

    options = {'user_data': (params, ptr),
...
"========= FIGURE 3 ZOOMED PLOT ========="

fig2, ax2 = plt.subplots()

# Create color gradient
colors = plt.cm.plasma(np.linspace(0.25, 1, params.npts_an+5))

# Plot only anode ionic phase potentials
for i in range(params.npts_an):

    ax2.plot(
        1e3 * solution.values.t,
        solution.values.y[:, ptr.phi_an_io[i]],
        color=colors[i],
        linewidth=2
    )
